"""Essay embeddings: RoBERTa (generated locally) and GPT (read from a saved file).

Ports of ``get_roberta_embeddings`` and ``get_gpt_embeddings`` from ``R/functions.R``.
...
``$LCP_DATA_ROOT/derived/roberta_embeddings.csv``. The pipeline reads that file with
:func:`read_roberta_embeddings`. torch and xgboost are never imported in the same
process (``llm_cong_predict.isolation``, docs/ORCHESTRATION.md). Pooling and batching
are tested on a tiny randomly initialised model; the real ``roberta-base`` weights
have not been run here. Install with ``pip install -e '.[embeddings]'``.
"""
...
def roberta_embeddings(
    ...
    """
    refuse_if_loaded("xgboost", "RoBERTa embedding generation")  # isolation.py
    try:
    ...
    # float32 model output held exactly as float64, as R stores keras' float32 predictions
    # in doubles (R: llm_paper/R/functions.R:L487).
    result = pd.DataFrame(emb.astype(np.float64), columns=cols)
    result.insert(0, "id", essays["ncdsid"].values)
    return result


def read_roberta_embeddings(path: str) -> pd.DataFrame:
    """Read the file written by the separate RoBERTa step (features/roberta_step.py):
    ``id`` + ``roberta_dim_1 .. roberta_dim_768``, the shape ``get_roberta_embeddings``
    returns (R: llm_paper/R/functions.R:L487–489). Imports neither torch nor xgboost."""
    df = pd.read_csv(path, dtype={"id": str})
    dims = [c for c in df.columns if c != "id"]
    expected = [f"roberta_dim_{i}" for i in range(1, len(dims) + 1)]
    if list(df.columns[:1]) != ["id"] or dims != expected:
        raise ValueError("RoBERTa embeddings file must have columns id, roberta_dim_1, ..., roberta_dim_<k>")
    return df
