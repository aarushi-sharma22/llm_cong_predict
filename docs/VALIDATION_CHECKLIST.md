...
| V10 | The minimum cell size, before anything is exported | every exported table | `config.MINIMUM_CELL_SIZE` is 10 (owner, Checkpoint D: the UK Data Service handling guide's baseline is 3, and 10 where several outputs come from one source, as here). **Confirm this with UKDS before releasing anything**, and then check what the export guard (PORTING_NOTES H4) refuses — appendix D8 (counts per aspired job) is the table most likely to hold small cells. Passing the guard is not a disclosure review. |
