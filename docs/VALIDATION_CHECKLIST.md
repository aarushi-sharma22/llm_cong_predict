...
| V10 | The minimum cell size, before anything is exported | every exported table | `config.MINIMUM_CELL_SIZE` ships as `None`, so the export guard (PORTING_NOTES H4) refuses every export. Confirm the threshold that applies to these data against the UK Data Service's output rules, set it, and then check what the guard refuses — appendix D8 (counts per aspired job) is the table most likely to hold small cells. Passing the guard is not a disclosure review. |
