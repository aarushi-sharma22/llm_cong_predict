...
Read from `reference/llm_paper/R/create_data.R` (601 lines, commit `b0cfe4c`) in full.
For each file the R writes: what it is built from, what it does to it,
...
| `appendix_D4_data.csv` | `appendix_1_data` (L241–242) | `ncds_essays` | `words` as numeric — nothing else | **participant-level: the full essay text with `ncdsid`** | yes, and it must never leave the secure area |
...
| `appendix_D7_data.csv` | `appendix_4_data` (L273–288) | `ncds_1_to_9`: the twelve BSAG totals | as numeric; long | **participant-level: every person's BSAG values with `ncdsid`** | yes, and it must never leave the secure area |
...
- **Aggregate tables that run in R** are ported as written, from the metric rows the
  ...
  `create_data.R`.
- **D4 and D7 are participant-level.** The port does not build them; it
...
- **D8** is aggregate but holds counts per aspired job; small cells are possible, which
  is the export guard's business.
